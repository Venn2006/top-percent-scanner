...
function getSpecializedManagerSkillLabels(jobTitle: string, industry?: string | null): { id: string; labels: string[] } | null {
  ...
  if (logisticsManagerRolePattern.test(text)) return { id: 'logistics-manager', labels: /truong kho|warehouse manager/.test(text) ? ['Ton kho, layout kho va cycle count', 'Nhap xuat, picking/packing va SLA ban giao', 'Team kho, ca truc va KPI kho', 'SOP kho, that thoat va exception dashboard'] : ['S&OP, demand/supply planning va OTIF', 'Carrier/vendor performance va cost-to-serve', 'Inventory accuracy, kho van va exception dashboard', 'Team SOP, escalation va continuous improvement'] };
