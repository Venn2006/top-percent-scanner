...
export default async function VerifyPage({ searchParams }: { searchParams: { [key: string]: string | undefined } }) {
  const { id, job, pct, date } = searchParams;
  
  // Kiểm tra trong DB purchases có vspi_id này không
      ...
      <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
        {isValid ? (
          ...
          <>
            ...
            <p className="text-sm text-slate-500">ID <strong className="font-mono">{id}</strong> không tồn tại hoặc chưa được xác thực.</p>
            <p className="text-[11px] text-slate-400 mt-3">Liên hệ: Zalo 0909.xxx.xxx</p>
          </>
        )}
      </div>
    </div>
